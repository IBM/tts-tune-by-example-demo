...
echo "" >> ./$timestamp$1-baseline/AudioList.txt
...
while IFS='' read -r line || [[ -n "$line" ]]; do
    ...
    # Setting the audio filename with the utterance text
    output_std_wav=$(echo $output4 | tr "," "_") &&
    echo "Processing tag " $line " into " $output_std_wav.wav " using voice " $voice &&
    echo "SSML Tag: " $line " -- TTS Standard Audio File " $output_std_wav.wav >> ./$timestamp$1-baseline/AudioList.txt &&
    curl -X POST -u $useCred --header "Content-Type: application/json" --header "Accept: audio/wav" --data "{\"text\":\"$line\"}" --output ./$timestamp$1-baseline/$output_std_wav-std.wav "$url/v1/synthesize?voice=$voice" &&
